...
const Notes:React.FC<Notes> = ({title,description,removeNote,editNote}) => {
    return (
        <div className = "relative font-tertiary text-xl w-[10.5rem] h-[10.5rem] p-1 md:w-52 md:h-52 bg-[#F3F369] space-2 z-10 drop-shadow-xl " id = "note">
            <h3 className = 'mt-2 ml-2 font-bold text-lg md:text-xl overflow-ellipsis w-[10.2rem] md:w-[12.6rem]'>{title}</h3>
            <p className = 'text-sm md:text-base ml-2 overflow-ellipsisw w-[9.8rem] md:w-[12.6rem]'>{description}</p>

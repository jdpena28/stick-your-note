...
const Modal:React.FC<modal|null> = ({trigger,click,title,desc,handleSubmit}) => {
    return trigger? (
      <div className = 'bg-white w-80 h-96 mx-auto rounded-[2rem] relative'>
        ...
        <form onSubmit = {handleSubmit}className = 'w-80 h-96 flex flex-col gap-y-3 align-center items-center justify-center'>
         <input type="text" name = 'text' ref = {title} id ='title' placeholder = 'Title' className ='w-[95%] border-[1px] border-gray-600 rounded-md p-1 mt-3' />
         <textarea name="description" ref = {desc} id="desc" className = 'border-[1px] border-gray-600 rounded-md p-1' cols = '40' rows = "8" placeholder = 'Description'></textarea>
